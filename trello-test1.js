  ...
  if (idOrg !== 0) {
    promiseGet("organizations/" + idOrg + "/members")
      .then(function (mbrs) {
        ...
        dust.render("templ5", {members: mbrs}, function (err, out) {
          console.log("Members: " + out);
          $("#members").empty();
          $('<ul>').append(out).appendTo("#members");
        });
      }).then(function () {
        ...
        var newBoards = boards.map(function (brd, idx) {
          ...
          boardMap[idx] = trelloObjects[brd.id]; // @@@@
...
var templ5 = '{#members}<li>'
    + '{fullName}'
    + '</li>{n}{/members}';
dust.loadSource(dust.compile(templ5, "templ5"));

var templ6 = '<table><thead><tr><td></td><td>Board</td><td>Original</td</tr></thead>{~n}'
    + '{#boards}<tr ><td>{$idx}</td>'
